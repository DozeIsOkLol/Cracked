        ...
        function()
            ...
            for _, g in pairs(P.PlayerGui:GetChildren()) do
                ...
                if s then
                    for j = 1, 3 do
                        for _, con in pairs(
                            getconnections(s[({"MouseButton1Click", "MouseButton1Down", "Activated"})[j]])
                        ) do
                            con:Fire()
                        end
                    end
            ...
            if not myTycoon then
                ...
                if ex then
                    ...
                    if col then
                        ...
                        local amt =
                            col:FindFirstChild("Display") and col.Display:FindFirstChild("Interface") and
                            col.Display.Interface:FindFirstChild("Amount")
                    ...
                    if btns then
                        ...
                        for _, b in pairs(btns:GetChildren()) do
                            ...
                            if b:FindFirstChild("Configuration") and b.Configuration:FindFirstChild("Price") then
                                if n:find("dropper") then
                                    hasDrop = true
                                elseif n:find("upgrader") then
                                    hasUpg = true
                                end
                            end
                        end
                        for _, b in pairs(btns:GetChildren()) do
                            if b:FindFirstChild("Configuration") and b.Configuration:FindFirstChild("Price") then
                                local n = b.Name:lower()
                                local tp =
                                    b:IsA("BasePart") and b or b:FindFirstChild("Head") or
                                    b:FindFirstChildWhichIsA("BasePart")
                                if tp and tp.Color ~= red then
                                    local canBuy = true
                                    if hasDrop then
                                        canBuy = n:find("dropper")
                                    elseif hasUpg then
                                        canBuy = n:find("upgrader")
                                    end
                                    if canBuy then
