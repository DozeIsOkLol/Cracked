...
local sP
local cartCosts={
[2]=15,[3]=100,[4]=300,[5]=800,[6]=3500,[7]=10000,[8]=24000,[9]=80000,
[10]=120000,[11]=250000,[12]=360000,[13]=700000,[14]=950000,[15]=1800000,
[16]=2500000,[17]=5000000,[18]=10000000,[19]=20000000,[20]=35000000,
[21]=90000000,[22]=130000000,[23]=180000000,[24]=350000000,[25]=500000000,
[26]=700000000,[27]=1000000000,[28]=2300000000,[29]=3500000000,
[30]=10000000000,[31]=18000000000,[32]=30000000000,[33]=70000000000,
[34]=150000000000,[35]=300000000000,[36]=670000000000
}
...
local addRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("Race"):WaitForChild("AddCoinsOrWins")
addRemote:FireServer(90,"Wins")
repeat task.wait(0.2) until winsStat.Value>=oldWins+90
local unlockRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("World"):WaitForChild("[C-S]UnlockNewWorld")
...
local upgradeRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("Tool"):WaitForChild("[C-S]UpgradeCarriagge")
for i=current+1,5 do
pcall(function() upgradeRemote:FireServer() end)
task.wait(0.1)
end
end
local buyTrailRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("Trail"):WaitForChild("[C-S]PlrWannaBuyTrail")
for i=1,10 do
pcall(function() buyTrailRemote:FireServer("trail"..i) end)
task.wait(0.05)
end
local equipTrailRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("Trail"):WaitForChild("[C-S]PlrWannaEquipTrail")
pcall(function() equipTrailRemote:FireServer("trail10") end)
...
repeat task.wait(0.3) until coinsStat.Value>=oldCoins+totalCartCost
local buyCartRemote=REP:WaitForChild("Remote"):WaitForChild("Function"):WaitForChild("CarShop"):WaitForChild("[C-S]BuyTool")
for _,cart in ipairs(cartsToBuy) do
local success=pcall(function() return buyCartRemote:InvokeServer(cart) end)
if success then
repeat task.wait(0.2) until P.BestTool.Value==cart
else
break
end
end
local equipRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("CarShop"):WaitForChild("[C-S]EquipCar")
pcall(function() equipRemote:FireServer("Cart36") end)
end
local buyCartRemote=REP:WaitForChild("Remote"):WaitForChild("Function"):WaitForChild("CarShop"):WaitForChild("[C-S]BuyTool")
pcall(function() buyCartRemote:InvokeServer("Robux Cart3") end)
...
local potionRemote=REP:WaitForChild("Remote"):WaitForChild("Event"):WaitForChild("Online"):WaitForChild("[C-S]IWannaGetThisReward")
...
pcall(function() luckRemote:InvokeServer(unpack(args)) end)
end
end
local idsToDelete={}
...
pcall(function() deleteRemote:InvokeServer(unpack(args)) end)
end
...
pcall(function() lockRemote:FireServer(unpack(args)) end)
end
end
end
end
...
for _,pet in ipairs(P.PlayerGui.mAIN.petbag.Frame.Frame1.ScrollingFrame:GetChildren()) do
if pet:FindFirstChild("cell") and pet.cell:FindFirstChild("id") then
if pet.Name=="Diamond Papagei" then diamondCount=diamondCount+1
elseif pet.Name=="Gold Papagei" then goldCount=goldCount+1
elseif pet.Name=="Papagei" then normalCount=normalCount+1
end
end
end
...
pcall(function() craftRemote:InvokeServer(unpack(args)) end)
...
for _,pet in ipairs(P.PlayerGui.mAIN.petbag.Frame.Frame1.ScrollingFrame:GetChildren()) do
if pet:FindFirstChild("cell") and pet.cell:FindFirstChild("id") and pet.Name=="Diamond Papagei" then
local petId=pet.cell.id.Value
local args={{
name="Diamond Papagei",
id=petId
}}
pcall(function() equipRemote:FireServer(unpack(args)) end)
equippedCount=equippedCount+1
if equippedCount>=3 then break end
end
end
end
end
