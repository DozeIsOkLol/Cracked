-- [[ 👑 HUMANOID PREMIUM V13 - STABLE & FAST ]] --
...
-- [[ 🖱️ SENİN ORİJİNAL HIZLI MAKRON (DOKUNULMADI) ]] --
task.spawn(function()
    while true do
        ...
        task.wait(0.01) -- Senin o meşhur hızın
    end
end)
...
local function ToggleFly()
    ...
    if _G.FlyActive then
        ...
        bg.P = 9e4; bg.maxTorque = Vector3.new(9e9, 9e9, 9e9); bg.Name = "H_FlyGyro"
        local bv = Instance.new("BodyVelocity", root)
        bv.maxForce = Vector3.new(9e9, 9e9, 9e9); bv.Name = "H_FlyVel"
        task.spawn(function()
...
btnMain.Size = UDim2.new(0.5, 0, 1, 0); btnMain.Text = "MAIN"; btnMain.TextColor3 = Color3.new(1,1,1); btnMain.Font = "GothamBold"; btnMain.BackgroundTransparency = 1

local btnFly = Instance.new("TextButton", tabs)
btnFly.Size = UDim2.new(0.5, 0, 1, 0); btnFly.Position = UDim2.new(0.5, 0, 0, 0); btnFly.Text = "FLY SETTINGS"; btnFly.TextColor3 = Color3.new(1,1,1); btnFly.Font = "GothamBold"; btnFly.BackgroundTransparency = 1

-- Pages
local pMain = Instance.new("Frame", main); pMain.Size = UDim2.new(1, 0, 0.7, 0); pMain.Position = UDim2.new(0, 0, 0.3, 0); pMain.BackgroundTransparency = 1
local pFly = Instance.new("Frame", main); pFly.Size = UDim2.new(1, 0, 0.7, 0); pFly.Position = UDim2.new(0, 0, 0.3, 0); pFly.BackgroundTransparency = 1; pFly.Visible = false

local function createB(txt, pos, parent)
    local b = Instance.new("TextButton", parent)
    b.Size = UDim2.new(0.85, 0, 0, 40); b.Position = pos; b.BackgroundColor3 = Color3.fromRGB(40, 40, 50); b.Text = txt; b.TextColor3 = Color3.new(1,1,1); b.Font = "GothamBold"
...
sBox.Size = UDim2.new(0.85, 0, 0, 40); sBox.Position = UDim2.new(0.075, 0, 0.4, 0); sBox.BackgroundColor3 = Color3.fromRGB(40, 40, 50); sBox.Text = "SPEED: 50"; sBox.TextColor3 = Color3.new(1,1,1); sBox.Font = "GothamBold"; Instance.new("UICorner", sBox)
...
btnMain.MouseButton1Click:Connect(function() pMain.Visible = true; pFly.Visible = false end)
btnFly.MouseButton1Click:Connect(function() pMain.Visible = false; pFly.Visible = true end)

-- Tıklama butonuna basınca olacaklar
cBtn.MouseButton1Click:Connect(function() 
    _G.AutoClick = not _G.AutoClick
    updateVisuals() 
end)

fBtn.MouseButton1Click:Connect(function() ToggleFly(); updateVisuals() end)
...
bBtn.MouseButton1Click:Connect(function()
    ...
    if _G.BypassActive then
        LP.Idled:Connect(function() VIM:SendKeyEvent(true, "Space", false, game) task.wait() VIM:SendKeyEvent(false, "Space", false, game) end)
    end
end)

sBox.FocusLost:Connect(function() _G.FlySpeed = tonumber(sBox.Text:match("%d+")) or 50; sBox.Text = "SPEED: ".._G.FlySpeed end)

-- [[ ⚙️ TUŞ ALGILAMA (BUG FIX) ]] --
UIS.InputBegan:Connect(function(i, c)
    if c then return end -- Chat açıksa çalışma
    if i.KeyCode == Enum.KeyCode.T then 
        _G.AutoClick = not _G.AutoClick 
        updateVisuals() -- T'ye basınca menüdeki yazıyı da günceller
    elseif i.KeyCode == Enum.KeyCode.Insert then 
        main.Visible = not main.Visible 
    end
end)

print("👑 HUMANOID PREMIUM V13 LOADED!")
